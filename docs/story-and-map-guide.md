...
| 顺序 | 任务 ID | 内容概要 | 完成后 |
|------|---------|----------|--------|
| 1 | `ch1_awakening` | 与守塔人交谈 → 到达裂隙区 | 自动接 `ch1_mirrors` |
| 2 | `ch1_mirrors` | 走访两处镜像标记点 | 自动接 `ch1_hearts` |
| 3 | `ch1_hearts` | 收集 2×僵尸之心（物品 7） | 自动接 `ch1_gate_td` |
| 4 | `ch1_gate_td` | 再与守塔人对话 | 解锁 `ch1_gate_td`，自动接 `ch1_return` |
| 5 | `ch1_return` | 收集虚空碎片（物品 40）→ 对话 | 解锁 `ch1_return` |

> **注意：** 任务链接替关系目前写在 `quest_manager.cpp` 的 `CompleteQuest()` 中（硬编码）。新增章节若需自动接取下一任务，须改代码或扩展为数据驱动。
...
| 项目 | 现状 |
|------|------|
| 任务自动链接 | 第一章链在 `CompleteQuest()` 硬编码，新章节需改 C++ 或改为数据驱动 |
