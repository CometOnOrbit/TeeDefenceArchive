...
public:
	...
	/** Writes path vertices to pVertices; at most MaxInner vertices, or returns 0 if unreachable / too long. */
	int GetPath(int VStart, int VEnd, vec2 *pVertices, int MaxInner = 1 << 20);
...
protected:
	...
	void *m_apBotPlaceholder[MAX_CLIENTS];
...
public:
	...
	void RegisterBot(int CID, CBot *pBot);
