...
namespace
{
	...
	static std::vector<std::string> BuildTuneCommands(const CTuneZoneManager &Mgr)
	{
		...
		for(const auto &[zoneType, zoneParams] : Mgr.m_Zones)
		{
			...
			{
				char aBuf[256];
				str_format(aBuf, sizeof(aBuf), "tune_zone %d %s %f", Mgr.GetZoneID(zoneType), CTuningParams::Name(tuneIdx), tuneValue);
				v.emplace_back(aBuf);
			}
		}
...
void CTuneZoneManager::LoadSoundsFromDirectory(const char *pRootDir, IStorage *pStorage)
{
	m_Sounds.clear();
	if(!pRootDir)
	{
	...
	for(int i = 0; i < SOUND_CUSTOM_COUNT; ++i)
	{
		...
		IOHANDLE File = pStorage->OpenFile(aFull, IOFLAG_READ, IStorage::TYPE_ALL);
		if(!File)
		{
			++missing;
...
std::optional<std::string> CTuneZoneManager::BakePreparedMap(const char *pMapName, IStorage *pStorage)
{
	...
	if(!Reader.Open(pStorage, pMapName, IStorage::TYPE_ALL))
	{
	...
	char aSourceHashLine[512];
	str_format(aSourceHashLine, sizeof(aSourceHashLine), "# sha256 %s", SourceSha256Str.c_str());

	std::vector<std::string> DeltaBody = vNewCommands;
	DeltaBody.emplace_back(aSourceHashLine);
	...
	auto PreparedIsUpToDate = [&](const std::string &Path) -> bool
	{
		...
		if(!Prep.Open(pStorage, Path.c_str(), IStorage::TYPE_ALL))
			return false;
	...
	pStorage->RemoveFile(PreparedPath.c_str(), IStorage::TYPE_SAVE);

	CDataFileWriter Writer;
	if(!Writer.Open(pStorage, PreparedPath.c_str()))
	{
